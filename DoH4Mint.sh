...
# Script interactivo para configurar DNS over HTTPS (DoH) en Linux Mint 21+
...
echo "🛡️  Configurador interactivo de DNS over HTTPS (DoH)"
echo ""
...
echo "🔧 Configurando DoH con: $DNS_ENTRY"
echo ""
...
if resolvectl status 2>/dev/null | grep -q "$DNS_ENTRY"; then
    echo "✅ ¡DoH configurado correctamente con $PROVIDER_NAME!"
else
